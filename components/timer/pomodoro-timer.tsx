...
export function PomodoroTimer({
  ...
  const [remainingTimes, setRemainingTimes] = useState<Record<TimerState, number>>({
  ...
  const handleTimerComplete = () => {
    ...
    if (timerState === "work") {
      ...
      const nextState = newCompletedPomodoros % 4 === 0 ? "long_break" : "short_break"
  ...
  useEffect(() => {
    setRemainingTimes((prev) => ({
      ...prev,
      work: timerStatus === "idle" && timerState === "work" ? getDuration("work") : prev.work,
      short_break:
        timerStatus === "idle" && timerState === "short_break" ? getDuration("short_break") : prev.short_break,
      long_break: timerStatus === "idle" && timerState === "long_break" ? getDuration("long_break") : prev.long_break,
    }))
  }, [workDuration, shortBreakDuration, longBreakDuration, timerState, timerStatus, getDuration])
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          {currentTask && (
            <div className="text-center space-y-2">
              <h3 className="font-medium text-lg text-balance">{currentTask.title}</h3>
          ...
          {/* Timer State Badge */}
          <Badge className={cn("px-4 py-2 text-sm font-medium", getStateColor())}>{getStateLabel()}</Badge>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground mt-1">
                  {timerStatus === "running" ? "Running" : timerStatus === "paused" ? "Paused" : "Ready"}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            {/* Previous State */}
            <Button variant="outline" size="icon" onClick={() => navigateTimer("prev")} className="h-12 w-12">
              <ChevronLeft className="h-5 w-5" />
            </Button>

            {/* Play/Pause */}
            <Button onClick={toggleTimer} size="lg" className={cn("h-14 w-14 rounded-full", getTimerColors().button)}>
              {timerStatus === "running" ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6" />}
            </Button>

            {/* Next State */}
            <Button variant="outline" size="icon" onClick={() => navigateTimer("next")} className="h-12 w-12">
            ...
            {Array.from({ length: 4 }, (_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i < completedPomodoros % 4 ? "bg-primary border-primary" : "border-muted-foreground",
                )}
              />
            ))}
            <span className="text-sm text-muted-foreground ml-2">{completedPomodoros} completed</span>
          </div>

          {/* No Task Warning */}
          {!currentTask && timerState === "work" && (
            <div className="text-center text-sm text-muted-foreground">Add a task to start your focus session</div>
          )}
        </div>
      </div>
    </div>
